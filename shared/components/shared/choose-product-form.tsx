...
export const ChooseProductForm: React.FC<Props> = ({ className, name, loading, items, price, imageUrl, onSubmit }) => {
  ...
  return (
    <div className={cn("flex flex-1 max-xl:flex-col", className)}>
      <div className="flex items-center justify-center flex-1 relative w-full m-0 p-0">
        <img 
          src={imageUrl} 
          alt={name} 
          className="relative transition-all z-10 duration-300 w-75 h-75 max-599:w-55 max-599:h-55"
        />
      </div>
